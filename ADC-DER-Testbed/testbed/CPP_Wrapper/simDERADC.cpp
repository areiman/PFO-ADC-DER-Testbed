...
//	        
//	if (time_current<5) {
//		fncs::publish("house0_l106_tm_heating_setpoint", "60");
//		fncs::publish("house0_l106_tm_cooling_setpoint", "68");
//	}
//	
//	else {
//		fncs::publish("house0_l106_tm_heating_setpoint", "61");
//		fncs::publish("house0_l106_tm_cooling_setpoint", "69");
//	}
	...
	ifstream myfile("CPP_PublishVar.txt"); // provide filename which contains keys to be published for GLD
	...
	//myValues.push_back(i*1+10); // This line is just for testing purpose. We need to delete once we integrate this with C++ Wrapper 
	...
	if (time_current<5) {
		fncs::publish(myKeys[i], "60");
		fncs::publish(myKeys[i+1], "68");
	}
	
	else {
		fncs::publish(myKeys[i], "61");
		fncs::publish(myKeys[i+1], "69");
	}
...
//-------------------------------------------------------------------------------------------------
        ...
        for(vector<int>::size_type i = 0; i != SubscribedValues.size(); i++) {
		cout<<"String is: "<< SubscribedString[i]<< " and Value is: " <<SubscribedValues[i]<<endl;
	}
